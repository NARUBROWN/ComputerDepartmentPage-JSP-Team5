...
<body>
	...
	<%
		int no_id = 0;
		if (request.getParameter("no_id") != null) {
			no_id = Integer.parseInt(request.getParameter("no_id"));
		}
		
		noticeDTO notice = new noticeDAO().getNotice(no_id);
	%>
		
	<section class="notice_table">
        <h2>공지사항 (수정)</h2>
        <form name="form1" method="POST" action="${pageContext.request.contextPath}/web_control.jsp">
        <input type="hidden" name="action" value="notice-update">
        <input type="hidden" name="no_id" value="<%= no_id %>">
            <ul>
                <li class="font_head">
                    <span>제목</span>
                    <span><input type="text" name="no_title" value="<%=notice.getNo_title() %>"></span>
                </li>
                <li class="notice_content">
                    <span>내용</span>
                    <span><textarea name="no_content"><%= notice.getNo_content() %></textarea></span>
                </li>
            </ul>
